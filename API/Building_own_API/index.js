...
app.get('/jokes/:id', (req, res) =>{
  ...
  const findJoke = jokes.find((a) => a.id===specificJoke);
  res.json(findJoke)
})

//3. GET a jokes by filtering on the joke type
app.get("/filter", (req, res) =>{
  const filterJoke= req.query.type;
  const findJoke = jokes.filter((a) => a.jokeType===filterJoke);
  res.json(findJoke)
})
...
app.put('/jokes/:id', (req, res) =>{
  ...
  const findIndex= jokes.findIndex((a)=> a.id === parseInt(req.params.id));
...
app.patch('/jokes/:id', (req, res) =>{
  ...
  const existingJoke= jokes.find((a)=> a.id===id);
  const replacingJoke={
    id:id,
...
app.delete('/jokes/:id', (req, res) =>{
  const id = parseInt(req.params.id);
    const existingJoke= jokes.findIndex((a)=> a.id===id);
    console.log(existingJoke);
    if(existingJoke !=-1){
      jokes.splice(existingJoke, 1);
      res.sendStatus(200);
    }
    else {
    res.status(404).json(`The Joke with the given ID:${id} was not found.`)
  }
})
